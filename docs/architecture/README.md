...
Body-edit-once contract: the first sync run rewrites the page's "System context" section to embed the three diagrams and inserts a sentinel comment (`ARCHITECTURE_DIAGRAMS_AUTOGENERATED`). Every run after that sees the sentinel and refreshes attachments only — humans editing prose elsewhere on the page are not overwritten. To restore the embed structure if it has been deleted manually, remove the sentinel from the page body and re-run the workflow.
